...
void drawGUI()
{
    if (click)
    {
        ...
        double Ox = (WINDOW_WIDTH + 1) / 2 + PIX_IN_CORD;
        double Oy = (WINDOW_HEIGHT + 1) / 2 - PIX_IN_CORD;

        for (int i = 0; i < WINDOW_WIDTH ; i += PIX_IN_CORD)
        {
            txLine(0, Oy + i, WINDOW_WIDTH, Oy + i);
            txLine(0, Oy - i, WINDOW_WIDTH, Oy - i);
        }
        for (int i = 0; i < WINDOW_HEIGHT ; i += PIX_IN_CORD)
        {
